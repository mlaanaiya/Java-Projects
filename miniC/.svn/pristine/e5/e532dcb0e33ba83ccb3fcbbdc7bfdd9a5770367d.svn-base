...
public class ConditionalExpression implements Expression {
	...
	public boolean collectAndBackwardResolve(HierarchicalScope<Declaration> _scope) {
		...
		boolean _bool = this.condition.collectAndBackwardResolve(_scope);
		boolean _result;
		if (this.elseExpression == null) {
			_result = _bool && this.thenExpression.collectAndBackwardResolve(_scope);
		} else {
			_result = _bool && this.thenExpression.collectAndBackwardResolve(_scope) && this.elseExpression.collectAndBackwardResolve(_scope);
		}
		return _result;
	}
	...
	public boolean fullResolve(HierarchicalScope<Declaration> _scope) {
		...
		boolean _bool = this.condition.fullResolve(_scope);
		boolean _result;
		if (this.elseExpression == null) {
			_result = _bool && this.thenExpression.fullResolve(_scope);
		} else {
			_result = _bool && this.thenExpression.fullResolve(_scope) 
	...
	public Type getType() {
		...
		if (this.condition.getType().compatibleWith(AtomicType.BooleanType)) {
			if (this.thenExpression.getType().compatibleWith(this.elseExpression.getType())) {
				...
				Logger.error("The type of then and else are incompatible.");
				_result = AtomicType.ErrorType;	
			}
		} else {
			Logger.error("The type of condition is incompatible.");
			_result = AtomicType.ErrorType;
		}
		return _result;
	}
